...
class RL_hessian():
    ...
    def __init__(self, model, batch_size=10, cuda=True):
        """
        ...
        loss = model.DANK_LOSS(batch_size)
        loss.backward(create_graph=True)

        # this step is used to extract the parameters from the model
        params, gradsH = get_params_grad(self.model.policy)
        self.params = params
        ...
        for k in range(n_v):
            ...
            for i in range(len(alpha_list)):
                ...
                if i < len(alpha_list) - 1:
                    T[i + 1, i] = beta_list[i]
                    T[i, i + 1] = beta_list[i]
            L_complex, V_complex = torch.linalg.eig(T)

            eigen_list = L_complex.real
            weight_list = V_complex.abs()**2
